...
const UpdateFormModal = ({ isOpen, onClose, onSubmit, initialData, isEdit, isLoading, treeName }: {
  ...
  const [form, setForm] = useState<TreeUpdatePayload>(EMPTY_FORM);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setForm(prev => ({ ...prev, [name]: name === 'photoUrl' || name === 'adminNotes' ? value : Number(value) }));
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-gray-900/60">
      <div className="bg-white rounded-t-2xl sm:rounded-xl w-full sm:max-w-lg shadow-2xl max-h-[95vh] overflow-y-auto">
        ...
        <form onSubmit={e => { e.preventDefault(); onSubmit(form); }} className="p-5 space-y-5">
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">URL Foto <span className="text-red-500">*</span></label>
            <input type="url" name="photoUrl" value={form.photoUrl} onChange={handleChange} required
              placeholder="https://storage.pohonku.id/trees/..."
              className="w-full px-3.5 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E562A]/20 focus:border-[#1E562A] transition-colors" />
            {form.photoUrl && (
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { name: 'heightCm',        label: 'Tinggi (cm)',   icon: Ruler, placeholder: '142.5' },
              { name: 'diameterCm',      label: 'Diameter (cm)', icon: Gauge, placeholder: '8.4'   },
              { name: 'co2AbsorbedTotal',label: 'CO₂ (kg)',      icon: Leaf,  placeholder: '24.5'  },
            ].map(({ name, label, icon: Icon, placeholder }) => (
              <div key={name}>
                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2">
                  <Icon className="w-3 h-3 inline mr-0.5" />{label} <span className="text-red-500">*</span>
                </label>
                <input type="number" name={name} value={(form as any)[name] || ''} onChange={handleChange}
                  required min={0} step={0.1} placeholder={placeholder}
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E562A]/20 focus:border-[#1E562A] transition-colors" />
              </div>
            ))}
          </div>
          ...
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
              <FileText className="w-3 h-3 inline mr-1" />Catatan Admin
            </label>
            <textarea name="adminNotes" value={form.adminNotes || ''} onChange={handleChange} rows={3}
              placeholder="Kondisi pohon, catatan penting, observasi lapangan..."
              className="w-full px-3.5 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1E562A]/20 focus:border-[#1E562A] transition-colors resize-none" />
          </div>

          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 border border-gray-200 rounded-lg text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-colors">Batal</button>
              ...
              <>
                <div className="relative flex-1">
                  ...
                  <input type="text" placeholder="Cari pohon..." value={searchQuery}
                    ...
                    className="w-full pl-8 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1E562A]/20 focus:border-[#1E562A] transition-colors" />
                </div>
