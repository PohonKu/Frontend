...
import {
  ...
  Clock, AlertCircle, ImageOff, ArrowLeft,
} from 'lucide-react';
...
const TimelineItem = ({ update, isLatest }: { update: TreeUpdateItem; isLatest: boolean }) => (
  <div className="flex gap-4">
    ...
    <div className="pb-6 flex-1 min-w-0">
      ...
      {/* Photo */}
      <div className="relative w-full h-48 sm:h-56 rounded-xl overflow-hidden bg-gray-100 mb-4 border border-gray-100">
        <Image
          src={update.photoUrl}
          alt="Foto pohon"
          fill
    ...
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="relative h-48 bg-gray-100">
        ...
        <div className="absolute bottom-3 left-3 right-3">
          <h3 className="text-white font-bold text-lg leading-tight">{adoption.species.name}</h3>
      ...
      <div className="px-4 py-3 border-b border-gray-50 grid grid-cols-3 gap-3 text-center">
        ...
        <div>
          <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wide">Update</p>
        ...
        <div>
          ...
          <p className="text-sm font-bold text-gray-900 mt-0.5 text-[11px]">{formatDateShort(adoption.expiresAt)}</p>
        </div>
      </div>
        ...
        {!hasUpdates ? (
          ...
          <div className="py-8 text-center">
            ...
            <p className="text-gray-400 text-xs mt-1 leading-relaxed max-w-[220px] mx-auto">
              Tim PohonKu sedang memantau pohonmu. Update perkembangan akan muncul di sini.
            </p>
          </div>
        ) : (
          <>
            {/* Latest update preview — always visible */}
            {latestUpdate && (
              <div>
              ...
              <div className="mt-5 pt-5 border-t border-gray-100">
                ...
                <div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="pb-2">
                      <p className="text-xs text-gray-400 mt-0.5">Mulai diadopsi · {formatDateShort(adoption.adoptedAt)}</p>
                    </div>
                  </div>
                </div>
              </div>
...
export default function TreeUpdatesPage() {
  ...
  const totalCO2 = adoptions.reduce((sum, a) => {
    const latest = a.tree.updates[0];
    return sum + (latest?.co2AbsorbedTotal ?? 0);
  }, 0);
        ...
        <div className="max-w-3xl mx-auto px-4 py-3.5 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-bold text-gray-900 flex items-center gap-2">
              <Leaf className="w-4 h-4 text-[#1E562A] shrink-0" />
              Perkembangan Pohonku
            </h1>
            <p className="text-[11px] text-gray-400 hidden sm:block">Riwayat update dari tim PohonKu</p>
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={label} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm text-center">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1 leading-tight">{label}</p>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-9 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1E562A]/20 focus:border-[#1E562A] transition-colors bg-white"
          />
          {searchQuery && (
        ...
        <div className="flex gap-2">
          {([
            ...
            { key: 'NONE', label: 'Belum Ada Update' },
        ...
        <p className="text-xs text-gray-400">
          Menampilkan <span className="font-bold text-gray-700">{filtered.length}</span> dari {adoptions.length} adopsi
        </p>
        ...
        {filtered.length === 0 ? (
          <div className="bg-white border border-gray-100 rounded-2xl p-12 text-center shadow-sm">
            ...
            <p className="text-gray-400 text-sm mt-1">
              {searchQuery ? 'Coba ubah kata kunci' : 'Belum ada adopsi pohon'}
            </p>
            {!searchQuery && (
              <Link href="/adopt"
          ...
          <div className="space-y-5">
            {filtered.map(adoption => (
              <AdoptionCard key={adoption.adoptionId} adoption={adoption} />
            ))}
          </div>
        )}
      </div>
